...
def main():
    ...
    tethered_tilt_analysis(args)
    plt.show()

def tethered_tilt_analysis(args):
    viz = FlightDataVisualizer(args.filename, args.output_folder, 10, 25)  # 02_AttitudeStabilization 
    viz.set_gains(K_x=[0.4, 0.4, 1.0], 
                  K_v=[0.8, 0.8, 2.0], 
                  K_R=[12.0, 12.0, 1.0, 0.0], 
                  K_W=[8.0, 8.0, 1.5])
    
    FlightDataVisualizer.x_cut_lines = [10.58, 12.58, 22.58, 24.58]
    ...
    Plotter.plot_vector3f_batch(t, (e_x, e_v, 10*thrusts), 
                    ...
                    limits=[[-0.7, 1.3], [-1.5, 1.3], [-8.0, 4.0]],
                    marker_style=["-", ":", "--"],
    ...
    Plotter.plot_vector3f(t, np.array([err_x, err_v, err_sum]).T, 
                    ...
                    title= "lya_pos_vel", limits=[[0, 0.9], [0, 1.5], [0, 2.0]], 
    ...
    Plotter.plot_vector3f_batch(t, (e_R, e_omega, 10*torques), 
                    ...
                    limits=[[-0.4, 0.5], [-0.4, 0.4], [-0.4, 0.4]],
                    marker_style=["-", ":", "--"],
